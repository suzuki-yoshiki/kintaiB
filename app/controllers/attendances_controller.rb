class AttendancesController < ApplicationController
  ...
  before_action :set_user, only: [:edit_one_month, :update_one_month, :edit_log, :apploval_one_month, :edit_overtime_info, :new_overtime_info, :new_overtime, :apploval_one_month_info, :change_request_info, :change_request, :apploval_request]
  ...
  def new_overtime_info
    ...
    @attendances = Attendance.where.not(finished_plan_at: nil).order("worked_on ASC")
    @superior = User.where(superior: true).where.not(id: current_user)
  end
  ...
  def change_request_info
    ...
    @attendances = Attendance.where.not(finished_at: nil).order("worked_on ASC")
  end
  ...
  private
    ...
    def attendances_params
      params.require(:user).permit(attendances: [:started_at, :finished_at, :note, :mark_change_confirmation, :change_confirmation, :tomorrow_at])[:attendances]
    end
    #勤怠変更申請の返信
    def change_request_params
      params.require(:user).permit(attendances: [:mark_change_confirmation, :change_at])[:attendances]
    end    
    #残業申請
    def overtime_params
    ...
    def apploval_request_params
      params.require(:user).permit(attendances: [:mark_apploval_confirmation, :apploval_confirmation, :apploval_month, :change_apploval])[:attendances]
    end
    # beforeフィルター
